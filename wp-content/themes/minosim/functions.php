...
function set_columns($columns)
{
    $columns = [
        ...
        'mang' => __('Nhà mang', 'minosim'),
        'goicuoc' => __('Gói cước', 'minosim'),
...
function column($column, $post_id)
{
    switch ($column) {
        ...
        case 'mang':
            echo getTenNhaMang(get_the_title($post_id), true);
            break;

        case 'goicuoc':
            echo get_post_field('goicuoc', $post_id);
            break;
        default:
            break;
    }
...
function agency_taxonomy_columns_content( $content, $column_name, $term_id )
{
    if ( 'count' == $column_name ) {
        ...
        global $simCounts;
        if (empty($simCounts)) {
            $simCounts = $wpdb->get_results( "SELECT agency, COUNT(*) as count FROM $wpdb->posts where post_type = 'sim' group by agency", OBJECT_K );
        }
        return isset($simCounts[$term_id]) ? number_format($simCounts[$term_id]->count) : 0;
    }
    return $content;
}
...
function get_type_sim($typesim = '', $nhamang = '', $dauso= '')
{
    ob_start();
    require('typesim.php');
    $form = ob_get_clean();
...
function custom_shortcode_atts_wpcf7_filter( $out, $pairs, $atts ) {
	...
	if ( isset( $atts[$my_goi_cuoc] ) ) {
        $out[$my_goi_cuoc] = $atts[$my_goi_cuoc];
    }
	if ( isset( $atts[$my_mang] ) ) {
        $out[$my_mang] = $atts[$my_mang];
    }
    return $out;
}
            ...
            <tbody>
                <tr class="alternate iedit">
                    ...
                    <td colspan="2" style="padding:8px 12px;">
                        <textarea style="width:100%;border-radius:4px;" name="sim" id="sim" rows="8" placeholder="Copy 4 cột SỐ, GIÁ, Loại thuê bao, Gói cước KM trong excel rồi paste vào đây &#10;Ví dụ: &#10;0999.999.999 50,000,000 tt c90&#10;0999.999.998 10,000,000 ts v90" required></textarea>
                    </td>
                </tr>
                <tr class="alternate iedit">
...
function add_sim_ajax() {
    ...
    foreach ($simArr as $key => $simso) {
        ...
        $loaithuebao = strtolower(trim($simso[2]));
        $goicuoc = trim($simso[3]);

        $dbso = remove_character($sso);
        if (strlen($dbso) == 9) {
            $dbso = '0' . $dbso;
            $sso = '0' . $sso;
        }
        ...
        $slug = strtolower($goicuoc) . '-' . $dbso;
        $valueSim .= "('$sso', '','','','','', '$slug', 'sim', 'publish', $sgia, $agency, '$loaithuebao', '$goicuoc', '$dbso', $authorId, '$time', '$time', '$time', '$time', 'closed', 'closed'),";
    }
    if ($valueSim) {
        $valueSim = substr($valueSim, 0, -1);
        $queryInsert = "INSERT INTO $wpdb->posts(post_title, post_content, post_excerpt, to_ping, pinged, post_content_filtered, post_name, post_type, post_status, price, agency, loaithuebao, goicuoc, simso, post_author, post_date, post_date_gmt, post_modified, post_modified_gmt, comment_status, ping_status) VALUES $valueSim";
...
function get_custom_theme_color() {
